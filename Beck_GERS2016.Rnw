...
%%%%%%
\begin{frame}{$\vcenter{\hbox{\includegraphics[width=0.05\paperwidth]{fig/epa_logo.png}}}$\hspace{0.07in}\textbf{Seagrasses and water quality}}
The algorithm was applied to entire estuaries with relevant data \\~\\
\begin{columns}
...
pdf('fig/tb_summ.pdf', height = 5, width = 13, family = 'serif')
grid.arrange(p1, p2, p3, p4, ncol = 4, nrow = 1, 
  widths = c(1, 0.35, 1, 0.35))
dev.off()
@
...
p3 <- ggplot(fortify(irl_seg), aes(long, lat)) + 
  ...
  scale_y_continuous() + 
  ylab('') +
  scale_size(name = '% SI', range = scls, breaks = leg_brks, 
    labels = leg_labs) + 
  scale_colour_gradientn(name = '% SI', 
    colours = pt_cols,
    breaks = leg_brks, 
...
pdf('fig/irl_summ.pdf', height = 4, width = 10, family = 'serif')
grid.arrange(p1, p2, p3, p4, ncol = 4, nrow = 1, 
  widths = c(1, 0.45, 1, 0.45))
dev.off()
@
...
\begin{frame}{$\vcenter{\hbox{\includegraphics[width=0.05\paperwidth]{fig/epa_logo.png}}}$\hspace{0.07in}\textbf{Seagrasses and water quality}}
Tampa Bay summary:\\~\\
\includegraphics[width = \textwidth]{fig/tb_summ.pdf}
Indian River Lagoon summary: \\~\\
\includegraphics[width = \textwidth]{fig/irl_summ.pdf}
\end{frame}
